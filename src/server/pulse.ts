...
export const pulseCreate = async () => {
	...
	try {
		execSync('pw-link TTMSpeaker:monitor_FL TTMMic:input_FL', { stdio: 'pipe' });
	} catch (error: any) {
		if (!error.stderr.toString().includes('failed to link ports: File exists')) {
			throw error;
		}
	}

	try {
		execSync('pw-link TTMSpeaker:monitor_FR TTMMic:input_FR', { stdio: 'pipe' });
	} catch (error: any) {
		if (!error.stderr.toString().includes('failed to link ports: File exists')) {
			throw error;
		}
